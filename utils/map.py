...
def filter_country(locations, country_df):
    ...
    deals_c = deals_c.loc[
        deals_c.loc[:, "country_low"].str.contains("ukraine", na=False), ["Deal ID"]
    ]
...
def top_subsidiaries(data, company, top_k):
    ...
    data_g = data_c.loc[:, ["Operating company: Name", "Deal size"]].groupby(
...
def top_parent(data, top_k):
    ...
    data_g = data.loc[:, ["Top parent companies", "Deal size"]].groupby(
